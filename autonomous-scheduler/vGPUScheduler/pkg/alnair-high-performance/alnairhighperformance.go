...
var (
	_ framework.FilterPlugin    = &alnairhighperformance{}
	_ framework.ScorePlugin     = &alnairhighperformance{}
	_ framework.ScoreExtensions = &alnairhighperformance{}
)

type alnairhighperformance struct {
	handle    framework.Handle
	clientset *kubernetes.Clientset
}

func New(_ runtime.Object, handle framework.Handle) (framework.Plugin, error) {
	klog.V(5).InfoS("Alnair vGPU Scheduling plugin for high performance is enabled")

	cs, err := clientsetInit()
	if err != nil {
		return nil, fmt.Errorf("alnair-high-performance cannot initialize in-cluster kubernetes config")
	}

	return &alnairhighperformance{
		handle:    handle,
		clientset: cs,
	}, nil
}

func (g *alnairhighperformance) Name() string {
	return Name
}
...
func (g *alnairhighperformance) Filter(ctx context.Context, state *framework.CycleState, pod *v1.Pod, node *framework.NodeInfo) *framework.Status {
...
func (g *alnairhighperformance) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
...
func (g *alnairhighperformance) NormalizeScore(_ context.Context, _ *framework.CycleState, pod *v1.Pod, scores framework.NodeScoreList) *framework.Status {
	var (
		highest int64 = 0
...
func (g *alnairhighperformance) ScoreExtensions() framework.ScoreExtensions {
	return g
}
