...
ALLUXIO_PASS      = os.getenv("ALLUXIO_PASS")
...
class ErrorManagedThread (threading.Thread):
    def run(self, *vargs, **kargs):
        self._exc = None
        try:
            super(ErrorManagedThread, self).run(*vargs, **kargs)
        except BaseException as e:
            self._exc = e
            raise e

    def join(self, timeout=None):
        super(ErrorManagedThread, self).join(timeout)
        if self._exc:
            raise self._exc
...
class MasterNodeTaskExecutor(ExecutorServiceABC):
    ...
    def get_space_needed_for_dataset_in_bytes(cls, dataset_path):
        ...
        try:
            val = (
                sp.run(
                    (f"sshpass -p {ALLUXIO_PASS} ssh alluxio-user@" + host_name + " du -s " + dataset_path).split(),
                    stdout=sp.PIPE,
                    stderr=None,
                    check=True,
                )
...
class WorkerNodeTaskExecutor(ExecutorServiceABC):
    ...
    def get_space_needed_for_dataset_in_bytes(self, dataset_path):
        ...
        try:
            val = (
                sp.run(
                    (f"sshpass -p {ALLUXIO_PASS} ssh alluxio-user@" + self._nodename + " du -s " + dataset_path).split(),
                    stdout=sp.PIPE,
                    stderr=None,
                    check=True,
                )
...
class AlluxioCacheManager(DatasetCacheManagerBase):
    ...
    def copy_data_to_datasetcache(self, datasetdir_basename, dataset_path, resource_rev):
        ...
        except BaseException as e:  # Also captures CalledProcessError when actual cmd fails
            warn(
    ...
    def _delete_original_resource(self, resource):
        ...
        try:
            sp.run(del_cmd.split(), stderr=None, stdout=None, check=True, timeout=60)

            log.info(f"Original resource {self._deploy_name} has been deleted")
...
def _notcalled_init_proxy_task():
