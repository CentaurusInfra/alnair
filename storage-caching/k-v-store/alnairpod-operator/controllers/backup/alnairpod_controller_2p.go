...
func (r *AlnairPodReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// create pod
	pod := &corev1.Pod{}
	if err := r.Get(ctx, types.NamespacedName{Name: fmt.Sprintf("%s-client", req.Name), Namespace: req.Namespace}, pod); err != nil && k8serrors.IsNotFound(err) {
		pods, err := r.createPod(ctx, alnairpod)
	...
	if !reflect.DeepEqual(alnairpod.Spec, oldspec) {
		...
		oldpod := corev1.Pod{}
		if err := r.Get(ctx, types.NamespacedName{Namespace: req.Namespace, Name: fmt.Sprintf("%s-client", req.Name)}, &oldpod); err != nil {
		...
		// create a new pod
		newpods, _ := r.createPod(ctx, alnairpod)
...
func (r *AlnairPodReconciler) createPod(ctx context.Context, alnairpod v1alpha1.AlnairPod) ([]corev1.Pod, error) {
	...
	client_container := corev1.Container{
		Name:            "client",
		Image:           ClientImage,
	...
	// containers = append(containers, client_container)

	// create the pod
	client_pod := corev1.Pod{
		TypeMeta: metav1.TypeMeta{
			Kind:       PodKind,
			APIVersion: APIVersion,
		},
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("%s-client", alnairpod.Name),
			Namespace: alnairpod.Namespace,
			UID:       types.UID(fmt.Sprintf("%s-worker", alnairpod.Name)),
			// Annotations: map[string]string{"k8s.v1.cni.cncf.io/networks": "macvlan-conf"},
		},
		Spec: corev1.PodSpec{
			Volumes:       volumes,
			Containers:    []corev1.Container{client_container},
			RestartPolicy: corev1.RestartPolicyNever,
			NodeSelector:  spec.NodeSelector,
			HostNetwork:   spec.HostNetwork,
		},
	}
	if err := ctrl.SetControllerReference(&alnairpod, &client_pod, r.Scheme); err != nil {
		return nil, err
	}
	...
	pod = corev1.Pod{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("%s-worker", alnairpod.Name),
			Namespace: alnairpod.Namespace,
			UID:       types.UID(fmt.Sprintf("%s-worker", alnairpod.Name)),
			// Annotations: map[string]string{"k8s.v1.cni.cncf.io/networks": "macvlan-conf"},
		},
	...
	return []corev1.Pod{client_pod, pod}, nil
}
